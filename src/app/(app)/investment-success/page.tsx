...
export default function InvestmentSuccessPage() {
    ...
    useEffect(() => {
        // Don't do anything until the user data is loaded
        if (loading || !user) return;

        const txId = sessionStorage.getItem('last_investment_tx');

        // If we have already found the transaction, do nothing.
        if (transaction) return;

        // If there is no transaction ID, the user should not be on this page.
        if (!txId) {
        ...
        if (foundTx) {
            // This is the success case. We found the transaction.
            setTransaction(foundTx);
            playSuccessSound();
            // We can now remove the ID from session storage.
            sessionStorage.removeItem('last_investment_tx');
        } else if (!loading) {
            // If still loading, we might just not have the tx yet.
            // If not loading, and tx not found, try to reload user data once.
            reloadUser();
        }
    }, [user, loading, router, playSuccessSound, transaction]);
